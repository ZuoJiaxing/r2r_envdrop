...
import utils
import model
from param import r2r_envdrop_args as R2R_envdrop_args
...
class BaseData(object):
    ...
    def rollout(self, **R2R_envdrop_args):
    ...
    def test(self, iters=None, **kwR2R_envdrop_args):
        ...
        if iters is not None:
            ...
            for i in range(iters):
                for traj in self.rollout(**kwR2R_envdrop_args):
                    self.loss = 0
...
class DataManager(BaseData):
    ...
    def _feature_variable(self, obs):
        ...
        features = np.empty((len(obs), R2R_envdrop_args.views, self.feature_size + R2R_envdrop_args.angle_feat_size), dtype=np.float32)
    ...
    def _candidate_variable(self, obs):
        ...
        candidate_feat = np.zeros((len(obs), max(candidate_leng), self.feature_size + R2R_envdrop_args.angle_feat_size), dtype=np.float32)
    ...
    def get_input_feat(self, obs):
        input_a_t = np.zeros((len(obs), R2R_envdrop_args.angle_feat_size), np.float32)
        for i, ob in enumerate(obs):
    ...
    def rollout(self, reset=True, feedback = 'teacher'):
        ...
        for t in range(self.episode_len):
            ...
            for i, next_id in enumerate(cpu_a_t):
                if next_id == (candidate_leng[i]-1) or next_id == R2R_envdrop_args.ignoreid:    # The last action is <end>
...
if __name__ == "__main__":
    ...
    train_env = R2RBatch(feature_data_store, batch_size=R2R_envdrop_args.batchSize, splits=['train'])
    data_manager = DataManager(train_env, "abc_tra.txt", R2R_envdrop_args.maxAction)
